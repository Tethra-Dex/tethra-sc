...
contract USDCPaymaster is Ownable, ReentrancyGuard {
    ...
    event PremiumUpdated(
        uint256 oldPremium,
        uint256 newPremium,
        uint256 timestamp
    );
    
    event ExecutorStatusUpdated(
    ...
    function processGasPayment(
        address user,
        uint256 gasUsed
    ) external nonReentrant returns (uint256 usdcCharged) {
     ...
     */
    function validateGasPayment(
        address user,
        uint256 estimatedGas
    ) external view returns (bool canPay) {
