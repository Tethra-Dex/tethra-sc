...
contract LimitExecutor is AccessControl, ReentrancyGuard {
    ...
    enum OrderType { LIMIT_OPEN, LIMIT_CLOSE, STOP_LOSS }
    ...
    function createLimitOpenOrder(
        ...
        // Collect collateral + trading fee + execution fee from trader
        uint256 positionSize = collateral * leverage;
        uint256 tradingFee = (positionSize * tradingFeeBps) / 10000;
        uint256 totalAmount = collateral + tradingFee + executionFee;
        
        require(
            usdc.transferFrom(msg.sender, address(treasuryManager), totalAmount),
            "LimitExecutor: Transfer failed"
        );
    ...
    function createLimitCloseOrder(
        ...
        require(trader == msg.sender, "LimitExecutor: Not position owner");
        require(status == 0, "LimitExecutor: Position not open");
        require(triggerPrice > 0, "LimitExecutor: Invalid trigger price");
        
        // Collect execution fee
        require(
            usdc.transferFrom(msg.sender, address(treasuryManager), executionFee),
            "LimitExecutor: Transfer failed"
        );
     ...
     */
    function executeLimitOpenOrder(
        uint256 orderId,
        ...
        require(
            keccak256(bytes(order.symbol)) == keccak256(bytes(signedPrice.symbol)),
            "LimitExecutor: Symbol mismatch"
        );
     ...
     * @param signedPrice Backend-signed price data
     */
    function executeLimitCloseOrder(
        uint256 orderId,
        SignedPrice calldata signedPrice
    ) external onlyRole(KEEPER_ROLE) nonReentrant {
        Order storage order = orders[orderId];
        ...
        (
            ...
            ,
            uint8 status
        ) = positionManager.getPosition(order.positionId);
        ...
        if (pnl > 0) {
            ...
            if (profit > tradingFee) {
                treasuryManager.refundCollateral(order.trader, collateral);
                treasuryManager.distributeProfit(order.trader, profit - tradingFee);
                treasuryManager.collectFee(order.trader, tradingFee);
            } else {
                treasuryManager.refundCollateral(order.trader, collateral + profit - tradingFee);
                treasuryManager.collectFee(order.trader, tradingFee);
            }
        } else if (pnl < 0) {
            uint256 loss = uint256(-pnl);
            if (collateral > loss + tradingFee) {
                treasuryManager.refundCollateral(order.trader, collateral - loss - tradingFee);
                treasuryManager.collectFee(order.trader, tradingFee);
            }
        } else {
            if (collateral > tradingFee) {
                treasuryManager.refundCollateral(order.trader, collateral - tradingFee);
                treasuryManager.collectFee(order.trader, tradingFee);
            }
        }
    ...
    /**
     * @notice Execute a stop-loss order (keeper only)
     * @param orderId Order ID to execute
     * @param signedPrice Backend-signed price data
     */
    function executeStopLossOrder(
        ...
        // Settlement logic (same as limit close)
        if (pnl > 0) {
            uint256 profit = uint256(pnl);
            if (profit > tradingFee) {
                treasuryManager.refundCollateral(order.trader, collateral);
    ...
    function cancelOrder(uint256 orderId) external nonReentrant {
        ...
        // If limit open order, refund collateral and trading fee
        if (order.orderType == OrderType.LIMIT_OPEN) {
            uint256 positionSize = order.collateral * order.leverage;
            uint256 tradingFee = (positionSize * tradingFeeBps) / 10000;
            treasuryManager.refundCollateral(msg.sender, order.collateral + tradingFee);
        }
    ...
    function _verifySignedPrice(SignedPrice calldata signedPrice) internal view {
        require(
            ...
            "LimitExecutor: Price expired"
        );
        require(
            signedPrice.timestamp <= block.timestamp,
            "LimitExecutor: Price timestamp in future"
        );
        ...
        require(
            ...
            "LimitExecutor: Invalid signature"
        );
    }
