...
contract TreasuryManager is AccessControl, ReentrancyGuard {
    ...
    event FeeDistributionUpdated(
        uint256 feeToLiquidity,
        uint256 feeToStaking,
    ...
    function updateFeeDistribution(
        uint256 _feeToLiquidity,
        uint256 _feeToStaking,
        uint256 _feeToTreasury
    ) external onlyRole(DEFAULT_ADMIN_ROLE) {
        require(
            _feeToLiquidity + _feeToStaking + _feeToTreasury == 10000,
            "TreasuryManager: Must sum to 10000 bps"
        );
