...
contract MarketExecutorTest is Test {
     ...
     */
    function _generateSignedPrice(string memory symbol, uint256 price, uint256 timestamp)
        internal
        view
        returns (MarketExecutor.SignedPrice memory)
    {
        ...
        return MarketExecutor.SignedPrice({symbol: symbol, price: price, timestamp: timestamp, signature: signature});
    }

    /**
     * @notice Generate signed price at current block time
     */
    function _generateCurrentSignedPrice(string memory symbol, uint256 price)
        internal
        view
        returns (MarketExecutor.SignedPrice memory)
    {
    ...
    function testIntegration_MultipleTraders() public {
        ...
        uint256 pos1 = executor.openMarketPosition(
            "BTC", true, COLLATERAL, LEVERAGE, _generateCurrentSignedPrice("BTC", BTC_PRICE)
        );
        ...
        vm.prank(trader2);
        uint256 pos2 =
            executor.openMarketPosition("ETH", false, COLLATERAL * 2, 5, _generateCurrentSignedPrice("ETH", ETH_PRICE));
